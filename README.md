...
- **Front-loaded cross-attn boost** — `xattn_boost` (`1.0` = off), norm-matched by default since v2.9. See [Cross-attn boost](#cross-attn-boost-front-loaded-text-adherence) below.
...
Since v2.9 the boost ships **norm-matched** by default (anima Phase-1'', `renorm='img'` ρ 0.5): the raw gain pushes hidden states off the norm distribution the next block was trained on (saturation burn / framing drift on complex prompts), so the boosted post-cross-attn state is rescaled back toward its unboosted per-image mean token norm — the boost becomes a *rotation toward the text direction* on the trained norm shell, while the token-norm peaks that carry highlights / neon survive intact.

- **`xattn_boost`** (both nodes) — λ. `1.0` = off (exact identity). **~1.5 recommended**; up to `3.0` for stubborn tags. Higher λ can amplify *unwanted* caption tags too (framing / crop priors ride along, and with style/artist tokens in the prompt the content win attenuates as the gain splits across tokens). It moves things the model already knows — a multiplicative lever on ~zero signal is still ~zero, so it will not conjure unknown concepts.
...
- **`xattn_boost_renorm`** (Advanced node only) — `img` (default; per-image mean-norm matching), `tok` (per-token — clamps exactly the highlight-peak tokens → flat grey tone; bench reference only), `off` (raw pre-v2.9 gain).
- **`xattn_boost_renorm_frac`** (Advanced node only) — partial-correction exponent ρ (`scale**ρ`); `1.0` = full shell match, `0.0` = raw boost. `0.5` at λ 2 was the validated tone sweet spot.
...
The boost is applied as an in-graph per-block buffer multiply (not a Python hook), so it is **`torch.compile` / AnimaBlockCompile safe** — the gain retunes per step with no recompile, the same buffer-read pattern the mod-guidance path uses. The norm matching needs the residual-add site inside `Block.forward`, so with `renorm != 'off'` the node additionally installs a faithful mirror of comfy predict2's `Block.forward` (bit-identical when neutral; gain buffer + static renorm flags read in-graph, same compile discipline). If ComfyUI's Block contract ever drifts, the mirror refuses to install and the boost falls back to the raw gain with a logged warning — never a crash or a silently divergent forward.
